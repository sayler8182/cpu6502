...
final class CPUExecuteBVCTests: XCTestCase {
    private var cpu: CPU!
    ...
    func test_BVC_REL() throws {
        let opcode = CPU.Instruction.BVC_OPCODE.REL
        cpu.flags.V = true
        cpu.PC = 0x01FD
    ...
    func test_BVC_REL_overflow_clear() throws {
        let opcode = CPU.Instruction.BVC_OPCODE.REL
        cpu.flags.V = false
        cpu.PC = 0x01DD
    ...
    func test_BVC_REL_overflow_clear_cross() throws {
        let opcode = CPU.Instruction.BVC_OPCODE.REL
        cpu.flags.V = false
        cpu.PC = 0x01FD
        memory[0x01FD] = opcode.byte
