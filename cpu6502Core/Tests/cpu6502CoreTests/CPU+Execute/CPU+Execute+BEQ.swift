...
final class CPUExecuteBEQTests: XCTestCase {
    private var cpu: CPU!
    ...
    func test_BEQ_REL() throws {
        let opcode = CPU.Instruction.BEQ_OPCODE.REL
        cpu.flags.Z = false
        cpu.PC = 0x01FD
    ...
    func test_BEQ_REL_zero_set() throws {
        let opcode = CPU.Instruction.BEQ_OPCODE.REL
        cpu.flags.Z = true
        cpu.PC = 0x01DD
    ...
    func test_BEQ_REL_zero_set_cross() throws {
        let opcode = CPU.Instruction.BEQ_OPCODE.REL
        cpu.flags.Z = true
        cpu.PC = 0x01FD
        memory[0x01FD] = opcode.byte
