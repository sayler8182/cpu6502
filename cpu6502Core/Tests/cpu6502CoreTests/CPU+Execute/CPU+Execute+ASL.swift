...
final class CPUExecuteASLTests: XCTestCase {
    private var cpu: CPU!
    ...
    func test_ASL_ACC() throws {
        let opcode = CPU.Instruction.ASL_OPCODE.ACC
    ...
    func test_ASL_ACC_carry() throws {
        let opcode = CPU.Instruction.ASL_OPCODE.ACC
        ...
        XCTAssertEqual(cpu.registers.A, 0b00001110)
        XCTAssertEqual(cycles, 2)
        XCTProgramCounter(pc: cpu.PC, size: opcode.size)
    ...
    func test_ASL_ZP() throws {
        let opcode = CPU.Instruction.ASL_OPCODE.ZP
        memory[CPU.START_PC] = opcode.byte
    ...
    func test_ASL_ZPX() throws {
        let opcode = CPU.Instruction.ASL_OPCODE.ZPX
        cpu.registers.X = 0x5
        memory[CPU.START_PC] = opcode.byte
        memory[CPU.START_PC + 1] = 0x42
    ...
    func test_ASL_ABS() throws {
        let opcode = CPU.Instruction.ASL_OPCODE.ABS
        memory[CPU.START_PC] = opcode.byte
    ...
    func test_ASL_ABSX() throws {
        let opcode = CPU.Instruction.ASL_OPCODE.ABSX
        cpu.registers.X = 0x5
        memory[CPU.START_PC] = opcode.byte
        memory[CPU.START_PC + 1] = 0x80
        memory[CPU.START_PC + 2] = 0x44 // 0x4480
