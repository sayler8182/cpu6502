...
final class CPUExecuteBPLTests: XCTestCase {
    private var cpu: CPU!
    ...
    func test_BPL_REL() throws {
        let opcode = CPU.Instruction.BPL_OPCODE.REL
        cpu.flags.N = true
        cpu.PC = 0x01FD
    ...
    func test_BPL_REL_negative_clear() throws {
        let opcode = CPU.Instruction.BPL_OPCODE.REL
        cpu.flags.N = false
        cpu.PC = 0x01DD
    ...
    func test_BPL_REL_negative_clear_cross() throws {
        let opcode = CPU.Instruction.BPL_OPCODE.REL
        cpu.flags.N = false
        cpu.PC = 0x01FD
        memory[0x01FD] = opcode.byte
