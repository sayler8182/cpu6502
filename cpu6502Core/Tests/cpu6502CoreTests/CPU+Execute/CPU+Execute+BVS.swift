...
final class CPUExecuteBVSTests: XCTestCase {
    private var cpu: CPU!
    ...
    func test_BVS_REL() throws {
        let opcode = CPU.Instruction.BVS_OPCODE.REL
        cpu.flags.V = false
        cpu.PC = 0x01FD
    ...
    func test_BVS_REL_overflow_set() throws {
        let opcode = CPU.Instruction.BVS_OPCODE.REL
        cpu.flags.V = true
        cpu.PC = 0x01DD
    ...
    func test_BVS_REL_overflow_set_cross() throws {
        let opcode = CPU.Instruction.BVS_OPCODE.REL
        cpu.flags.V = true
        cpu.PC = 0x01FD
        memory[0x01FD] = opcode.byte
