...
final class CPUExecuteBNETests: XCTestCase {
    private var cpu: CPU!
    ...
    func test_BNE_REL() throws {
        let opcode = CPU.Instruction.BNE_OPCODE.REL
        cpu.flags.Z = true
        cpu.PC = 0x01FD
    ...
    func test_BNE_REL_zero_clear() throws {
        let opcode = CPU.Instruction.BNE_OPCODE.REL
        cpu.flags.Z = false
        cpu.PC = 0x01DD
    ...
    func test_BNE_REL_zero_clear_cross() throws {
        let opcode = CPU.Instruction.BNE_OPCODE.REL
        cpu.flags.Z = false
        cpu.PC = 0x01FD
        memory[0x01FD] = opcode.byte
