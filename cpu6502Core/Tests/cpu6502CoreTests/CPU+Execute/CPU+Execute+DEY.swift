...
final class CPUExecuteDEYTests: XCTestCase {
    private var cpu: CPU!
    ...
    func test_DEY_IMPL() throws {
        let opcode = CPU.Instruction.DEY_OPCODE.IMPL
        ...
        XCTAssertEqual(cpu.registers.Y, 0x36)
    ...
    func test_DEY_IMPL_overflow() throws {
        let opcode = CPU.Instruction.DEY_OPCODE.IMPL
        ...
        XCTAssertEqual(cpu.registers.Y, 0xFF)
        XCTAssertEqual(cycles, 2)
        XCTProgramCounter(pc: cpu.PC, size: opcode.size)
