...
final class CPUExecuteBCSTests: XCTestCase {
    private var cpu: CPU!
    ...
    func test_BCS_REL() throws {
        let opcode = CPU.Instruction.BCS_OPCODE.REL
        cpu.flags.C = false
        cpu.PC = 0x01FD
    ...
    func test_BCS_REL_carry_set() throws {
        let opcode = CPU.Instruction.BCS_OPCODE.REL
        cpu.flags.C = true
        cpu.PC = 0x01DD
    ...
    func test_BCS_REL_carry_set_cross() throws {
        let opcode = CPU.Instruction.BCS_OPCODE.REL
        cpu.flags.C = true
        cpu.PC = 0x01FD
        memory[0x01FD] = opcode.byte
